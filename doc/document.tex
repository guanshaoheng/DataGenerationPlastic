...
\begin{document}
	...
	\begin{algorithmic}[1]
		...
		\State $k++$
		\EndWhile
		\If{$f(\sigma^{(n+1, k)}, H(\bar{\epsilon^{p}}^{(k)})) < -f_{tolerance}$} \Comment{Avoiding overestimating plasticity}
		\State \Call{Searching for the right scalar of plastic strain via dichotomy}{$\mathrm{d}\lambda$}
		\EndIf
